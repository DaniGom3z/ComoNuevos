    ...
    const handleEliminarCitaFisicamente = async (idCita) => {
        try {
          ...
          if (token) {
            ...
            fetchData();
            console.log('respuesta exitosa', response.data);
            alert('Eliminaste físicamente la cita');
          } else {
            console.log('el token no se guardo pai');
          }
        } catch (error) {
        ...
        <div className="flex flex-grow flex-wrap items-start pt-20 pl-10 pr-10 justify-center">
          <table className="relative w-full top-32 bg-transparent border rounded-md">
            <thead className="bg-transparent border-b">
              <tr>
                <th className='p-2 w-full absolute bg-transparent text-2xl -top-16 text-left'>Papelera de Citas</th>
              </tr>
              <tr>
                ...
                {citas ? (
                  citas.map((cita) => (
                    <tr className='text-center border' key={cita.id_cita}>
                      ...
                      <td className=' border flex p-1 space-x-1 '>
                        <button
